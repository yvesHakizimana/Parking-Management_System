...
void setup() {
  ...
  Serial.println("🛑 RFID Card Wipe Mode");
...
/*
 * Wipes all writable blocks by filling them with spaces (0x20)
 */
bool wipeCard() {
  byte block;
  byte emptyBlock[16];
  ...
  for (int i = 0; i < 16; i++) {
    emptyBlock[i] = 0x20;  // Fill block with ASCII spaces
  }
  ...
  for (block = 1; block < 64; block++) {
    ...
    if (status != MFRC522::STATUS_OK) {
      ...
      Serial.print("✅ Block ");
      Serial.print(block);
      Serial.println(" wiped (filled with spaces).");
    }
  }
